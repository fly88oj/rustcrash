...
| tailscale.go | `proto/tailscale.rs` + `tailscale/{noise,controlhttp,derp}.rs` — wave-9 config surface + wave-10 WIRE FOUNDATIONS: the controlbase Noise IK_25519_ChaChaPoly_BLAKE2s handshake + framed transport, the controlhttp `/ts2021` upgrade client, and the DERP client (5-byte frames, ServerInfo registration, packet relay) — all mimic-driven end-to-end | 🟡 (ipn state machine + netstack wiring remain; connect still reports the precise remaining gap) |
| easytier.go | `proto/easytier.rs` — full config surface + the WHOLE of mihomo component/easytier ported (TOML render/validation, peer-URI parsing with peer-public-key, required-flags forcing, overlay DNS/PTR/magic-DNS lookup, upstream test vectors); connect fails with the staged first milestone (the Rust mesh core is not in-tree) | 🟡 wave-10 (config+helpers ✅; mesh core staged) |
...
| ech.go | `proto/ech.rs` + `reality/tls13.rs::connect_ech` — full runtime on the engine's own TLS 1.3 stack: inner/outer ClientHello shaping (empty wire session id, compat-ext drop, [0x01] marker), HPKE-sealed extension, accept/HRR confirmations, transcript rebind to the inner hello, inner-name cert verify, rejection → `ech_required`/`tls: server rejected ECH` + one retry with the EE retry configs; wired on vless/trojan/vmess/anytls (`ech-opts`) | ✅ wave-8 TCP-TLS (QUIC/h2 carriers — hy2/tuic/trusttunnel — still blocked by the quinn/rustls TLS hook, precise error; HTTPS-RR discovery needs DNS type-64, precise error) |
...
| P2 | ECH on QUIC carriers; snell-listener stacked frontings (shadow-tls/res-tls/jls-in-snell); anytls-listener TLS fronting; tailscale ipn | precise-error scopes (see rows above); tlsmirror h2 generator ✅ wave-10 |
